...
/*creating a custom hook called useSignup (line 8) and exporting it,
...
export const useSignUp = () => {
  ...
  /*so the pieces of state that we have is for an error and if there is an error with the signup request,
  ...
  //creating a signup function inside the useSignup hook (line 19)//
  const signup = async (username, password) => {
    //inside the async function we take in the username and password (line 19) that the user signs up with//

    //Now inside the signup function we set isLoading to true (line 23)//
    setIsLoading(true);
    setError(null);
    ...
    and inside it we use '/api/user/register'(line 33) the reason we not using localhost port 9000 is,
